...
let createDomElementTwo = (ele, className='', text='', id='', attribute1='', attribute2='', html='', role='') => {
    let elementSemantic = document.createElement(ele);
    if (className) {
        let breakClassName = className.split(':');
        breakClassName.forEach((classToAdd) => {
...
let convertTableToMobile = () => {
    ...
    getTables.forEach((table) => {
        ...
        var fullTable = table;
        let $parentContainer = createDomElementTwo(ele = 'div', className = 'parentContainer', '', 'mobile-container', 'aria-labelledby:mobile-caption', 'aria-describedby:infoText', '', 'group');
